...
if use_NP:
    Basecase_fixed_results_path = f'D-2_CGM_perfect_NP_results_{GSK}/' + str(Number_of_hours) + '_hours'
    if not os.path.exists(Basecase_fixed_results_path):
        os.makedirs(Basecase_fixed_results_path)
    #load predictions
    predictions = pd.read_csv(f'ML_results\\{Number_of_hours}_hours\\comparison_NP.csv', index_col=0)

    # Getting "Actual-values" for the perfect CGMA
    predictions = predictions.drop(columns=['Predicted'])

    # pivot: index = Timestep, columns = Line, value = Actual
    predictions = predictions.pivot(index='Timestep', columns='Line', values='Actual')

    # columns = ['1', '2', '3']
    predictions.columns = [str(i) for i in predictions.columns]
...
if use_NP:
    MC_fixed_results_path = f'D-1_MC_perfect_NP_results_{GSK}/' + str(Number_of_hours) + '_hours'
    if not os.path.exists(MC_fixed_results_path):
        os.makedirs(MC_fixed_results_path)
    d_2_np = pd.read_csv(f'D-2_CGM_perfect_NP_results_{GSK}\\{Number_of_hours}_hours\\df_np.csv', index_col="Time Step")
    d_2_line_f = pd.read_csv(f'D-2_CGM_perfect_NP_results_{GSK}\\{Number_of_hours}_hours\\df_line_f.csv', index_col="Time Step")
...
# These sum up to 0
zone_exports = flows_with_zones.groupby(level="ZoneFrom", axis=1).sum()
zone_imports = flows_with_zones.groupby(level="ZoneTo", axis=1).sum()
...
zone_exports_2 = flows_with_zones_updated.groupby(level="ZoneFrom", axis=1).sum()
zone_imports_2 = flows_with_zones_updated.groupby(level="ZoneTo", axis=1).sum()
...
Basecase_fixed_results_path = f'D-1_CGM_perfect_NP_results_{GSK}\\{Number_of_hours}_hours'
if not os.path.exists(Basecase_fixed_results_path):
    os.makedirs(Basecase_fixed_results_path)
...
if use_NP:
    generation = pd.read_csv(f'D-1_MC_perfect_NP_results_{GSK}\\{Number_of_hours}_hours\\d_1_gen.csv', index_col=0)
    curtailment = pd.read_csv(f'D-1_MC_perfect_NP_results_{GSK}\\{Number_of_hours}_hours\\d_1_curt.csv', index_col=0)
