...
max_ntc = 1000
Number_of_days = 30*6  # Number of days to consider
...
if use_NP:
    Basecase_fixed_results_path = f'D-2_base_case_fixed_NP_results_{GSK}/' + str(Number_of_hours) + '_hours'
...
if use_NP:
    MC_fixed_results_path = f'D-1_MC_fixed_NP_results_{GSK}/' + str(Number_of_hours) + '_hours'
    if not os.path.exists(MC_fixed_results_path):
        os.makedirs(MC_fixed_results_path)
    d_2_np = pd.read_csv(f'D-2_base_case_fixed_NP_results_{GSK}\\{Number_of_hours}_hours\\df_np.csv', index_col="Time Step")
    d_2_line_f = pd.read_csv(f'D-2_base_case_fixed_NP_results_{GSK}\\{Number_of_hours}_hours\\df_line_f.csv', index_col="Time Step")
...
Basecase_fixed_results_path = f'D-1_CGM_fixed_NP_results_{GSK}\\{Number_of_hours}_hours'
if not os.path.exists(Basecase_fixed_results_path):
    os.makedirs(Basecase_fixed_results_path)

#PTC_mapping = pd.read_csv(f'D-1_MC_results\\{Number_of_hours}_hours\\PTC_mapping.csv', index_col=0)

if use_NP:
    generation = pd.read_csv(f'D-1_MC_fixed_NP_results_{GSK}\\{Number_of_hours}_hours\\d_1_gen.csv', index_col=0)
    curtailment = pd.read_csv(f'D-1_MC_fixed_NP_results_{GSK}\\{Number_of_hours}_hours\\d_1_curt.csv', index_col=0)
    predictions = pd.read_csv(f'ML_results\\{Number_of_hours}_hours\\predictions_NP.csv', index_col=0)
...
for t in T: 
    for l in L:
        if df_line_f.loc[t, l] > get_line_cap(l):
          #  print(f"Line {l} has a flow of {df_line_f.loc[t, l]} at time {t} which is above the capacity of {get_line_cap(l)}")
            overload_df.loc[t, l] = df_line_f.loc[t, l] - get_line_cap(l)
        if df_line_f.loc[t, l] < -get_line_cap(l):
           # print(f"Line {l} has a flow of {df_line_f.loc[t, l]} at time {t} which is below the capacity of {-get_line_cap(l)}")
            overload_df.loc[t, l] = df_line_f.loc[t, l] + get_line_cap(l)
