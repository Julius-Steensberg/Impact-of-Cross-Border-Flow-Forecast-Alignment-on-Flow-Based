...
if use_PTC:
    Basecase_fixed_results_path = 'D-2_CGM_perfect_PTC_results/' + str(Number_of_hours) + '_hours'
    if not os.path.exists(Basecase_fixed_results_path):
        os.makedirs(Basecase_fixed_results_path)
    #load predictions
    line_f_flow_D1_CGM = pd.read_csv(f'D-1_CGM_results_flat\\{Number_of_hours}_hours\\df_line_f.csv', index_col=0)
    all_branch_ids = PTC_mapping["BranchID"].explode().tolist()
    flat_string_list = [str(item) for sublist in map(ast.literal_eval, all_branch_ids) for item in sublist]

    #based on flat_string_list, extract the columns from line_f_flow_D1_CGM
    line_f_flow_D1_CGM = line_f_flow_D1_CGM[flat_string_list]
    PTC_from_basecase_D1_CGM = pd.DataFrame(index=line_f_flow_D1_CGM.index, columns=PTC_mapping["PTC"].unique())

    for col in PTC_from_basecase_D1_CGM.columns:
        branch_ids = PTC_mapping[PTC_mapping["PTC"] == col]["BranchID"].explode().tolist()  
        branch_ids = [str(item) for sublist in map(ast.literal_eval, branch_ids) for item in sublist]
        PTC_from_basecase_D1_CGM[col] = line_f_flow_D1_CGM[branch_ids].sum(axis=1)
#make a copy of PTC_from_basecase_D1_CGM
predictions = PTC_from_basecase_D1_CGM.copy(deep=True)
predictions_from_ML = pd.read_csv(f'ML_results\\{Number_of_hours}_hours\\predictions_PTC_50_epochs.csv', index_col=0)

#only take index from preditions that are in predictions_from_ML
predictions = predictions.loc[predictions.index.isin(predictions_from_ML.index)]
...
if use_PTC:
    MC_fixed_results_path = 'D-1_MC_perfect_PTC_results/' + str(Number_of_hours) + '_hours'
    if not os.path.exists(MC_fixed_results_path):
        os.makedirs(MC_fixed_results_path)
    d_2_np = pd.read_csv(f'D-2_CGM_perfect_PTC_results\\{Number_of_hours}_hours\\df_np.csv', index_col="Time Step")
    d_2_line_f = pd.read_csv(f'D-2_CGM_perfect_PTC_results\\{Number_of_hours}_hours\\df_line_f.csv', index_col="Time Step")
...
Basecase_fixed_results_path = 'D-1_CGM_perfect_PTC_results_flat/' + str(Number_of_hours) + '_hours'
if not os.path.exists(Basecase_fixed_results_path):
    os.makedirs(Basecase_fixed_results_path)

#load predictions
if use_PTC:
    generation = pd.read_csv(f'D-1_MC_perfect_PTC_results\\{Number_of_hours}_hours\\d_1_gen.csv', index_col=0)
    curtailment = pd.read_csv(f'D-1_MC_perfect_PTC_results\\{Number_of_hours}_hours\\d_1_curt.csv', index_col=0)
