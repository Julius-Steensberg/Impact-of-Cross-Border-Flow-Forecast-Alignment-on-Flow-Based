...
#load predictions
predictions = pd.read_csv(f'ML_results\\{Number_of_hours}_hours\\predictions_NP.csv', index_col=0)
comparison = pd.read_csv(f'ML_results\\{Number_of_hours}_hours\\comparison_NP.csv', index_col=0)
PTC_mapping = pd.read_csv(f'D-1_MC_results_{GSK}\\{Number_of_hours}_hours\\PTC_mapping.csv', index_col=0)

comparison['CGMA Prediction Error'] = comparison['Actual'] - comparison['Predicted']

# Pivot the comparison_NP dataframe
CGMA_prediction_error = comparison.reset_index().pivot(
    index='Timestep', columns='Line', values='CGMA Prediction Error')
cols_CGMA_pred_error = ['1', '2', '3']
CGMA_prediction_error.columns = cols_CGMA_pred_error

# change all negative values in CGMA_prediction_error to 0.001
CGMA_prediction_error_neg = CGMA_prediction_error.copy(deep = True)
CGMA_prediction_error_neg[CGMA_prediction_error_neg > 0] = 0
CGMA_prediction_error_neg[CGMA_prediction_error_neg < 0] = 0.001

# change all positive values in CGMA_prediction_error to -0.001
CGMA_prediction_error_pos = CGMA_prediction_error.copy(deep = True)
CGMA_prediction_error_pos[CGMA_prediction_error_pos < 0] = 0
CGMA_prediction_error_pos[CGMA_prediction_error_pos > 0] = -0.001

uncertainty = CGMA_prediction_error_neg + CGMA_prediction_error_pos

#%%

for column in predictions.columns:

    # add 0.001 to predictions of the column
    predictions[column] = predictions[column] + uncertainty[column]

    print(f"Adding uncertainty to: NP{column}")

    Basecase_fixed_results_path = f'D-2_PD_{GSK}/' + str(Number_of_hours) + '_hours_NP' + column
    ...
    d_2_NP_slack_pos = np.zeros((len(T), len(predictions.columns)), dtype=float)  # PTC slack variables (positive)
    d_2_NP_slack_neg = np.zeros((len(T), len(predictions.columns)), dtype=float)  # PTC slack variables (negative)  
    ...
    for horizon in range(1, ceil(len(T) / hours_per_horizon) + 1):  # Python range is inclusive of start and exclusive of end
        ...
        prediction_NP_slack_pos = m.addVars(
            [(t, l) for t in Tsub for l in predictions.columns],
            lb=0,
            ub=GRB.INFINITY,
            name="NP_slack_pos"    
        )
        prediction_NP_slack_neg = m.addVars(
            [(t, l) for t in Tsub for l in predictions.columns],
            lb=0,
            ub=GRB.INFINITY,
            name="NP_slack_neg"
        ...
        m.setObjective(
            ...
            sum(prediction_NP_slack_pos[t, l]*cost_curt*1.1 + prediction_NP_slack_neg[t, l]*cost_curt*1.1 for t in Tsub for l in predictions.columns),
            GRB.MINIMIZE
        )
        ...
        for t in Tsub:
            for z in Z_FBMC:
                ...
                # Add the constraint
                m.addConstr(
                    NP[t, z]  == sum(NOD_INJ[t, n] for n in n_in_z[z]) +sum(EXPORT[t, zz] for zz in zz_list),                            
        ...
        for t in Tsub:
            for z in Z_FBMC:
                m.addConstr(
                    NP[t, z] == predictions.loc[t, z] + prediction_NP_slack_pos[t, z] - prediction_NP_slack_neg[t, z],
                    name=f"prediction_{t}_{z}"
                )
        ...
        for t_idx, t in enumerate(Tsub):
            ...
            for l_idx, l in enumerate(predictions.columns):
                d_2_NP_slack_pos[start_index+t_idx, l_idx] = prediction_NP_slack_pos[t, l].X
                d_2_NP_slack_neg[start_index+t_idx, l_idx] = prediction_NP_slack_neg[t, l].X
    ...
    df_NP_slack_pos = pd.DataFrame(d_2_NP_slack_pos, index=T, columns=predictions.columns)
    df_NP_slack_pos.index.name = "Time Step"
    df_NP_slack_pos.columns.name = "Line"

    # Create DataFrame for LINE_F_slack_neg
    df_NP_slack_neg = pd.DataFrame(d_2_NP_slack_neg, index=T, columns=predictions.columns)
    df_NP_slack_neg.index.name = "Time Step"
    df_NP_slack_neg.columns.name = "Line"
    ...
    df_NP_slack_neg.to_csv(f'{Basecase_fixed_results_path}\\df_NP_slack_neg.csv')
    df_NP_slack_pos.to_csv(f'{Basecase_fixed_results_path}\\df_NP_slack_pos.csv')
    ...
    if use_NP:
        MC_fixed_results_path = f'D-1_MC_PD_{GSK}/' + str(Number_of_hours) + '_hours_NP' + column
        if not os.path.exists(MC_fixed_results_path):
            os.makedirs(MC_fixed_results_path)
        d_2_np = pd.read_csv(f'{Basecase_fixed_results_path}\\df_np.csv', index_col="Time Step")
        d_2_line_f = pd.read_csv(f'{Basecase_fixed_results_path}\\df_line_f.csv', index_col="Time Step")
    T = d_2_np.index
    ...
    for horizon in range(1, ceil(len(T) / hours_per_horizon) + 1):  # Python range is inclusive of start and exclusive of end
        ...
        m.setObjective(
            ...
            sum(EXPORT[t, z, zz]*0.0000001 for t in Tsub for z in Z for zz in z_to_z[z]),   #very small cost for EXPORT to avoid flows in both directions
            GRB.MINIMIZE
        )
    ...
    #make all columns in all_cross_border_flows to be strings
    all_cross_border_flows.columns = all_cross_border_flows.columns.astype(str)
    ...
    net_positions_with_adjacent_zones.columns = net_positions_with_adjacent_zones.columns.astype(str)
    ...
    Basecase_fixed_results_path = f'D-1_CGM_PD_{GSK}/' + str(Number_of_hours) + '_hours_NP' + column
    if not os.path.exists(Basecase_fixed_results_path):
        os.makedirs(Basecase_fixed_results_path)

    PTC_mapping = pd.read_csv(f'D-1_MC_results_{GSK}\\{Number_of_hours}_hours\\PTC_mapping.csv', index_col=0)

    if use_NP: # use MC_fixed_results_path \\d_1_gen.csv
        generation = pd.read_csv(f'{MC_fixed_results_path}/d_1_gen.csv', index_col=0)
        curtailment = pd.read_csv(f'{MC_fixed_results_path}/d_1_curt.csv', index_col=0)        
        predictions = pd.read_csv(f'ML_results\\{Number_of_hours}_hours\\predictions_NP.csv', index_col=0)
