...
T = T[:24*Number_of_days]  # Limit the time steps to the first 60 days
...
d_2_objective_per_hour = np.zeros(len(T), dtype=float)  # Objective function value per hour
...
for horizon in range(1, ceil(len(T) / hours_per_horizon) + 1):  # Python range is inclusive of start and exclusive of end
    ...
    start = (horizon - 1) * hours_per_horizon + 1
    start_index = (horizon - 1) * hours_per_horizon
    end = min(horizon * hours_per_horizon + (days_foresight - 1) * 24, len(T))
    Tsub = list(range(start, end + 1))  # Create the range of time steps for the current horizon
    ...
    print("Built constraints line_cap_pos for all lines.")
    ...
    print("Built constraints line_cap_neg for all lines.")
...
df_objective_per_hour = pd.DataFrame(d_2_objective_per_hour, index=T, columns=["Objective"])
...
print(f"Running time D2 Base-case: {int(hours):02}:{int(minutes):02}:{int(seconds):02}")

D2_results_path = f'D-2_base_case_results_{GSK}/' + str(Number_of_hours) + '_hours'
if not os.path.exists(D2_results_path):
    os.makedirs(D2_results_path)

# save df_export to csv in results folder
df_export.to_csv(f'{D2_results_path}/df_export.csv')
df_np.to_csv(f'{D2_results_path}/df_np.csv')   
df_gen_costs.to_csv(f'{D2_results_path}/df_gen_costs.csv')
df_gen.to_csv(f'{D2_results_path}/df_gen.csv')
df_line_f.to_csv(f'{D2_results_path}/df_line_f.csv')
df_nodal_price.to_csv(f'{D2_results_path}/df_nodal_price.csv')
df_nod_inj.to_csv(f'{D2_results_path}/df_nod_inj.csv')
df_delta.to_csv(f'{D2_results_path}/df_delta.csv')
df_curt.to_csv(f'{D2_results_path}/df_curt.csv')
df_curt_costs.to_csv(f'{D2_results_path}/df_curt_costs.csv')
df_objective_per_hour.to_csv(f'{D2_results_path}/df_objective_per_hour.csv')
...
df_export = pd.read_csv(f'{D2_results_path}/df_export.csv', index_col="Time Step")
d_2_np = pd.read_csv(f'{D2_results_path}/df_np.csv', index_col="Time Step")
d_2_line_f = pd.read_csv(f'{D2_results_path}/df_line_f.csv', index_col="Time Step")
...
d_1_dual_variable_ram_pos = np.zeros((T_len, len(CNEC)), dtype=float)  # Dual variable for RAM_pos
d_1_dual_variable_ram_neg = np.zeros((T_len, len(CNEC)), dtype=float)  # Dual variable for RAM_neg
...
for t in T:
    for j in CNEC:
        ...
        RAM_pos[(t, j)] = get_line_cap(j) * (1 - frm) - d_2_line_f[t - 1, L.index(j)]
        RAM_neg[(t, j)] = -get_line_cap(j) * (1 - frm) - d_2_line_f[t - 1, L.index(j)]
...
for horizon in range(1, ceil(len(T) / hours_per_horizon) + 1):  # Python range is inclusive of start and exclusive of end
    ...
    # add constraints == objective value per hour
    for t in Tsub:
        m.addConstr(
            OBJECTIVE_PER_HOUR[t] == sum(GEN[t, p] * get_mc(p) for p in P) +
        ...
        for j in CNEC:
            m.addConstr(
                sum(
                    ...
                    (NP[t, z_fb] - d_2_np[t-1, Z.index(z_fb)])
                    for z_fb in Z_FBMC
                )
                <= RAM_pos[(t, j)],
    ...
    for t_idx, t in enumerate(Tsub):
        ...
        # Dual variables for RAM_pos and RAM_neg
        for j_idx, j in enumerate(CNEC):
            d_1_dual_variable_ram_pos[start_index+t_idx, j_idx] = m.getConstrByName(f"flow_on_cnes_pos_{t}_{j}").Pi
            d_1_dual_variable_ram_neg[start_index+t_idx, j_idx] = m.getConstrByName(f"flow_on_cnes_neg_{t}_{j}").Pi
...
D1_results_path = f'D-1_MC_results_{GSK}/' + str(Number_of_hours) + '_hours'
if not os.path.exists(D1_results_path):
    os.makedirs(D1_results_path)
...
# Convert the dual variable RAM_pos and RAM_neg to DataFrames
d_1_dual_variable_ram_pos_df = pd.DataFrame(d_1_dual_variable_ram_pos, columns=CNEC, index=range(1, d_1_dual_variable_ram_pos.shape[0] + 1))
d_1_dual_variable_ram_neg_df = pd.DataFrame(d_1_dual_variable_ram_neg, columns=CNEC, index=range(1, d_1_dual_variable_ram_neg.shape[0] + 1))

# Save the DataFrames to CSV files
d_1_curt_df.to_csv(f'{D1_results_path}/d_1_curt.csv')
d_1_np_df.to_csv(f'{D1_results_path}/d_1_np.csv')
d_1_gen_df.to_csv(f'{D1_results_path}/d_1_gen.csv')
d_1_nodal_price_df.to_csv(f'{D1_results_path}/d_1_nodal_price.csv')
d_1_obj_per_hour_df.to_csv(f'{D1_results_path}/d_1_obj_per_hour.csv')   
d_1_dual_variable_ram_pos_df.to_csv(f'{D1_results_path}/d_1_dual_variable_ram_pos.csv')
d_1_dual_variable_ram_neg_df.to_csv(f'{D1_results_path}/d_1_dual_variable_ram_neg.csv')

# Write out PTDFs to CSV
PTDF_Z_CNEC_df.to_csv(f'{D1_results_path}/PTDF_Z_CNEC.csv')
    ...
    for z in Z_FBMC:  # Iterate over FB zones
        for zz in Z_not_in_FBMC:  # Iterate over non-FBMC zones
            exports_fb_to_non_fb.append({
                ...
                "Zone From": z,
                "Zone To": zz,
                #"Export": EXPORT[t, z, zz].X if (t, z, zz) in EXPORT else 0.0  # Get value or default to 0.0
    ...
    for z in Z_FBMC:  # Iterate over FB zones
        for zz in Z_not_in_FBMC:  # Iterate over non-FBMC zones
            exports_non_fb_to_fb.append({
                ...
                "Zone From": zz,
                "Zone To": z,
                #"Export": EXPORT[t, zz, z].X if (t, zz, z) in EXPORT else 0.0  # Get value or default to 0.0
                "Export": export_df.loc[
                (export_df["Time Step"] == t) &
...
all_cross_border_flows.to_parquet(f'{D1_results_path}/Y_line.parquet')
...
PTC_flows.to_parquet(f'{D1_results_path}/Y_PTC.parquet')
net_positions_FBMC.to_parquet(f'{D1_results_path}/Y_NP_FBMC.parquet')
net_positions_with_adjacent_zones.to_parquet(f'{D1_results_path}/Y_NP_with_adjacent_zones.parquet')
...
#final_line_mapping_table.to_csv("results\\PTC_mapping.csv", index=True)
final_line_mapping_table.to_csv(f'{D1_results_path}/PTC_mapping.csv', index=True)

# export df_ram as parquet
df_ram.to_parquet(f'{D1_results_path}/RAM.parquet')
